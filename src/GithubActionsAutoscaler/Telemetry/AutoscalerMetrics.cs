...
namespace GithubActionsAutoscaler.Telemetry;
...
public class AutoscalerMetrics
{
	...
	public AutoscalerMetrics(Meter meter, IQueueProvider queueProvider, IRunnerManager runnerManager)
	{
		...
		meter.CreateObservableGauge(
			...
			() => new Measurement<long>(
				queueProvider.GetApproximateMessageCountAsync().GetAwaiter().GetResult()
			)
		);
		meter.CreateObservableGauge(
			"autoscaler.runners.active",
			() => new Measurement<long>(
				runnerManager.GetRunnerCountAsync().GetAwaiter().GetResult()
			)
		);
	}
